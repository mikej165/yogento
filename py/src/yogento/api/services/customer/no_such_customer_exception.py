...
class NoSuchCustomerException(Exception):
    class Builder:
        ...
        def build(self):
            return NoSuchCustomerException(increment_id=self.__increment_id)

        def set_increment_id(self, increment_id):
            self.__increment_id = increment_id
            return self

        def update(self, no_such_customer_exception):
            if isinstance(no_such_customer_exception, NoSuchCustomerException):
                self.set_increment_id(no_such_customer_exception.increment_id)
            elif isinstance(no_such_customer_exception, dict):
                for key, value in no_such_customer_exception.iteritems():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(no_such_customer_exception)
            return self
    ...
    def __repr__(self):
        ...
        return 'NoSuchCustomerException(' + ', '.join(field_reprs) + ')'
    ...
    def write(self, oprot):
        oprot.writeStructBegin('NoSuchCustomerException')
