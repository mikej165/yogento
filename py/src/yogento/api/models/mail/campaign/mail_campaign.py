...
class MailCampaign(object):
    class Builder:
        ...
        def build(self):
            return MailCampaign(id=self.__id, mail_chimp_campaign=self.__mail_chimp_campaign, status=self.__status)
    ...
    def __hash__(self):
        return hash((self.id,self.mail_chimp_campaign,self.status,))
    ...
    def as_dict(self):
        return {'id': self.id, 'mail_chimp_campaign': self.mail_chimp_campaign, 'status': self.status}
    ...
    def replace(self, id=None, mail_chimp_campaign=None, status=None):
        if id is None:
            id = self.id
        ...
        if status is None:
            status = self.status
        return self.__class__(id=id, mail_chimp_campaign=mail_chimp_campaign, status=status)
